...
perturbs the joints around that pose, captures the board at each one, then
hands control to you for N_MANUAL extra poses spread across the workspace,
returns home, solves with every OpenCV hand-eye method, drops outlier poses,
and writes cam2base.json.

Manual phase controls (in the preview window):
    R      release the servos so you can move the arm by hand
           (HOLD THE ARM FIRST -- it goes limp)
    SPACE  lock the servos and capture this pose
    Q      finish the manual phase early
Set N_MANUAL = 0 to skip the manual phase entirely.
...
N_POSES = 20                # automatic poses, including home
N_MANUAL = 10               # hand-positioned poses after the automatic ones (0 = skip)
...
# =====================================================================
# Manual phase -- you position the arm by hand, script captures
# =====================================================================
def _lock_servos(mc):
    """Re-engage all servos so the arm holds its current pose."""
...
def collect_manual(mc, pipeline, board, dictionary, K, dist, n_target=N_MANUAL):
    """Live preview; R releases the arm, SPACE locks it and captures.
    Returns a list of samples tagged source='manual'."""
    samples = []
    released = False
    font = cv2.FONT_HERSHEY_SIMPLEX

    print(f"\nMANUAL PHASE: {n_target} poses. Spread them across the picking zone")
    print("(near/far, left/right, high/low) and tilt the board differently each time.")
    print("  R = release arm (HOLD IT FIRST)   SPACE = lock + capture   Q = finish\n")
    ...
    try:
        while len(samples) < n_target:
            ...
            ch_corners, ch_ids = _detect_charuco(gray, board, dictionary)
            ok = ch_corners is not None and len(ch_ids) >= MIN_CORNERS

            vis = color.copy()
            if ch_corners is not None:
                cv2.aruco.drawDetectedCornersCharuco(vis, ch_corners, ch_ids)
            cv2.putText(vis, f"manual {len(samples)}/{n_target}  "
                             f"{'BOARD OK' if ok else 'no board'}",
                        (10, 30), font, 0.7, (0, 255, 0) if ok else (0, 0, 255), 2)
            state = ("RELEASED: pose arm, SPACE = lock + capture" if released
                     else "LOCKED: hold arm, R = release")
            cv2.putText(vis, state, (10, 60), font, 0.6, (255, 255, 0), 2)
            cv2.putText(vis, "Q = finish manual phase", (10, 88), font, 0.55,
                        (200, 200, 200), 1)
            cv2.imshow("manual calibration", vis)
            key = cv2.waitKey(1) & 0xFF

            if key == ord('q'):
                break

            if key == ord('r') and not released:
                mc.release_all_servos()
                released = True
                print("  servos released -- move the arm")
                ...
                angles = _get_angles(mc)
                if angles is None:
                    print("    no joint readback, try again")
                    continue
                det = _capture_board(pipeline, board, dictionary, K, dist)
                if det is None:
                    print("    board not detected / poor fit here -- "
                          "release (R) and adjust the pose")
                    continue
                samples.append({"joint_angles_deg": angles, "source": "manual", **det})
                print(f"  [manual {len(samples)}/{n_target}] {det['n_corners']} corners, "
                      f"reproj {det['reproj_px']} px, angles {np.round(angles, 1)}")
    finally:
        if released:                    # never leave the arm limp
            _lock_servos(mc)
            time.sleep(SETTLE_S)
    ...
    try:
        ...
        for i, tgt in enumerate(targets, 1):
            ...
            if det is None:
                ...
                continue
            samples.append({"joint_angles_deg": actual, "source": "auto", **det})
            print(f"    ok: {det['n_corners']} corners, reproj {det['reproj_px']} px")

        if N_MANUAL > 0:
            samples += collect_manual(mc, pipeline, board, dictionary, K, dist)
...
def solve(urdf_path=URDF_PATH):
    ...
    sources = [s.get("source", "auto") for s in samples]
    dropped = []
    ...
    if bad.any() and (~bad).sum() >= MIN_SAMPLES:
        ...
        n_man = sum(sources[i] == "manual" for i in dropped)
        print(f"\ndropping {len(dropped)} outlier pose(s) {dropped} "
              f"({n_man} manual), re-solving:")
        rms, name, T_c2b, resid, centroid = _best_method(
            [T_g2b[i] for i in keep], [T_t2c[i] for i in keep])

    kept_src = np.array([sources[i] for i in keep])
    for src in ("auto", "manual"):
        m = kept_src == src
        if m.any():
            print(f"  {src:6s} poses kept: {m.sum():2d}   "
                  f"RMS {np.sqrt((resid[m] ** 2).mean()) * 1000:6.2f} mm")
